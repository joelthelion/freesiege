...
Menu::Menu(const Titles &titles,const std::string &ttf_path,TextureIds ids) {
	//font creation
	...
	for (Titles::const_iterator title=titles.begin(); title!=titles.end(); title++) {
		...
		normal_surf=TTF_RenderText_Solid(normal_font,title->c_str(),normal_color);
		selected_surf=TTF_RenderText_Solid(selected_font,title->c_str(),selected_color);

		title_sprite->spr_normal=new Sprite(normal_surf,*ids++);		
		title_sprite->spr_selected=new Sprite(selected_surf,*ids++);
